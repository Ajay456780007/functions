...
def Proposed_model(x_train,x_test,y_train,y_test,percentage,epochs):
    ...
    print(f"Training with {percentage}% data ")
    checkpoint_epochs = [100, 200, 300, 400, 500]
    checkpoint_results = {}
    all_y_true = []
    all_y_pred = []

    for epoch in range(500):  # always train for 500 epochs
        model.train()
        total_loss, correct, total = 0, 0, 0

        for x_seq, x_graph, y, G_small in train_loader:
            x_seq, x_graph, y, G_small = x_seq.to(device), x_graph.to(device), y.to(device), G_small.to(device)
        ...
        print(f"Epoch {epoch + 1} | Train Accuracy: {train_acc:.2f}% | Loss: {total_loss / len(train_loader):.4f}")
        ...
        with torch.no_grad():
            for x_seq, x_graph, y, G_small in test_loader:
                x_seq, x_graph, y, G_small = x_seq.to(device), x_graph.to(device), y.to(device), G_small.to(device)
                outputs = model(x_seq, x_graph, G_small)
                _, predicted = torch.max(outputs.data, 1)
        ...
        if (epoch + 1) in checkpoint_epochs:
            ...
            filename = f"Analysis/Performance_Analysis/Zea_mays/metrics_{int(percentage)}percent_epoch{epoch + 1}.npy"
            np.save(filename, metrics)
            print(f"[📁] Metrics saved to {filename}")

            # Clear metrics except for the final checkpoint
            if (epoch + 1) != 500:
